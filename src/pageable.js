...
;(function (root, factory) {
  ...
  function () {
      ...
      if (this.firstInit) {
        ...
        this.container =
          typeof this.container === 'string'
            ...
            : this.container // container not found
        ...
        this.pages.forEach(function (page, i) {
          ...
          if (anchor) {
            ...
            if (
              Array.isArray(that.config.anchors) &&
              that.config.anchors.length
            ) {
    ...
    Pageable.prototype.update = function () {
      ...
      this.wrapper.style['padding-' + (this.horizontal ? 'bottom' : 'right')] =
        this.bar + 'px' // reset scroll position (do this AFTER setting dimensions)
    ...
    Pageable.prototype._wheel = function (e) {
      ...
      if (this.events.wheel && !this.scrolling) {
        ...
        inc
          ? this.index < this.pages.length - 1 && index++
          : 0 < this.index && index--
    ...
    Pageable.prototype._getData = function () {
      var scrolled = this.config.infinite
        ? this.scrollPosition - this.data.window[this.size[this.axis]]
        : this.scrollPosition
      var max = this.config.infinite
    ...
    Pageable.prototype._overScroll = function (inc) {
      ...
      if (index === this.lastIndex && inc) {
        ...
        this._scrollBy(
          -this.data.window[this.size[this.axis]] - scrolled,
          index
        )
      } else if (index === 0 && !inc) {
        index--
      ...
      that.timer = setTimeout(
        function () {
          ...
          var scroll = function scroll () {
            ...
            var scrolled = that.config.easing(
              ct,
              start,
              amount,
              that.config.animation
            )
            that.container.style.transform = that.horizontal
              ? 'translate3d(' + scrolled + 'px, 0, 0)'
              : 'translate3d(0, ' + scrolled + 'px, 0)'
